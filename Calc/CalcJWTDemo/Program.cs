...
namespace CalcJWTDemo
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            _httpClient.DefaultRequestHeaders.Add("Ocp-Apim-Subscription-Key", "<your API MGMT API key>");
            _httpClient.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("MyConsoleApp", "1.0"));

            var request = new HttpRequestMessage()
            {
                RequestUri = new Uri("https://<yourAPIMInstanceName>.azure-api.net/calc/api/Calc/Add?a=23&b=5")
            };
